    ...
    public partial class frmConsultaBugs : Form
    {
        ...
        private void btnConsultar_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(cboEstados.Text))
            {
                ...
                strSql += "AND (id_estado=@idEstado) "; // esto evita hacer una concatenacion de datos
                // xq idEstado es un parametro, es la variable de arriba, si no tendria que utilizar lo de las comillas
                // "''"
            ...
            if (!string.IsNullOrEmpty(cboCriticidades.Text))
            {
                ...
                strSql += "AND (id_criticidad=@idCriticidades) ";
                parametros.Add("idCriticidades", idCriticidades);
            }

            if (!string.IsNullOrEmpty(cboProductos.Text))
            {
                var idProductos = cboProductos.SelectedValue.ToString();
                strSql += "AND (id_producto=@idProductos) ";
                parametros.Add("idProductos", idProductos);
            }
        ...
        private void cboAsignadoA_SelectedIndexChanged(object sender, EventArgs e)
        {
